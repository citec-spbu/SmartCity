...
def create_wide_flat_hilly_dem(output_file, width, height, cell_size, base_elevation, max_elevation_change, sigma, block_size, noise_reduction_factor):
    """
    Создает DEM файл с широкими и низкими холмами.

    Args:
        output_file (str): Путь к выходному файлу .dem.
        width (int): Ширина DEM в пикселях.
        height (int): Высота DEM в пикселях.
        cell_size (float): Размер ячейки DEM в единицах карты.
        base_elevation (float): Базовая высота.
        max_elevation_change (float): Максимальное изменение высоты.
        sigma: Параметр для сглаживания гауссовым фильтром.
        block_size: Размер блока для создания разреженного шума.
        noise_reduction_factor: Множитель для уменьшения силы шума.
    """
    ...
    # ... (остальная часть кода аналогична)
